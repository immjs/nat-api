...
export async function createUpnpClient(options: { sourcePort?: number, interface?: string } = {}) {
  const ssdp = await createSsdp(options);
...
export class UpnpClient {
  ...
  async map(options: UpnpMapOptions) {
    ...
    if (remotePort === undefined) {
      throw new Error(
        `The options.public should assign the port of the port to expose!`,
      );
    }
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(function () {
        reject(
          Object.assign(
            new Error(
              'Fail to find gateway. Maybe your router does not support upnp!',
            ),
